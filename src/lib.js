...
export function ucFirstWord(string) {
  ...
  // If acronym the words should be all uppercase
  var acronyms = [ 'faq', ]
  ...
  if (acronyms.includes(word)) {
    ...
    words.forEach((word, index) => {
      if (index > 0 && index < words.length && /\b(the|of|in)\b/.test(word)) {
        arr.push(word)
      } else {
        arr.push(ucFirst(word))
      }
    })
    return arr.join(' ')
  }
...
export function validateDisplayName(string) {
  const name = string.toLowerCase().replace(/\s+/, '')

  if (name.length > 3) {
    return 'Display name must be at least 4 characters in length!'
  } else if (!name.search(unicodeCharacters())) {
...
export function extend(comment) {
  const someText = comment => {
    if ((comment.text.match(/\n/g) || []).length >= 10) { // more than 10 newlines?
      ...
      return comment.text.slice(0, Math.min(i, 200)).replace(/\n/g, '<br>')
    } else if (comment.text.length > 200) {
      return comment.text.slice(0, 200).replace(/\n/g, '<br>')
    }
  }
  ...
  const obj = {
    ...
    someText: someText(comment),
    score: comment.likes - comment.dislikes,
    createdAt: createdDate,
    createdText: dateString(createdDate),
  }
  return obj
}

export function sortHot(c1, c2) {
  // https://medium.com/hacking-and-gonzo/how-reddit-ranking-algorithms-work-ef111e33d0d9
  const hotScore = (comment) => {
    const order = Math.log10(Math.max(Math.abs(comment.score), 1))
    let sign
    if (comment.score > 0) sign = 1
    else if (comment.score === 0) sign = 0
    else sign = -1

    // 10 hours older = -1 hotscore = x1/10 score
    const score = sign * order - (Date.now() - comment.createdAt.getTime()) / (3600 * 1000 * 10)
    return score
  }
  return hotScore(c2) - hotScore(c1)
}
export const sortNew = (c1, c2) => c2.createdAt.getTime() - c1.createdAt.getTime()
export const sortTop = (c1, c2) => c2.score - c1.score
export const sortPrio = (c1, c2) => {
  if (c1.prio === c2.prio) return 0
